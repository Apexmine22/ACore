...
public class HoverChat {
    ...
    public static void sendHover(final String message, final String show_text) {
        if (HoverChat.enable) {
            ...
            if (Bukkit.getOnlinePlayers().size() == 0) {
                Bukkit.getServer().sendPluginMessage((Plugin) ACore.getInstance(), "BungeeCord", b.toByteArray());
            }
            else {
                ((Player)Iterables.getFirst((Iterable)Bukkit.getOnlinePlayers(), (Object)null)).sendPluginMessage((Plugin) ACore.getInstance(), "BungeeCord", b.toByteArray());
            }
        }
        else {
    ...
    static {
        HoverChat.enable = ACore.config.getBoolean("Settings.bungee_module_enable");
    }
}
