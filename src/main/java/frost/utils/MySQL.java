...
public class MySQL extends Thread {
    ...
    public static boolean hasAccountBan(final String playerName) {
        try {
            return ACore.sql.getConnection().createStatement().executeQuery("SELECT * FROM `Ban` WHERE `username`='" + playerName.toLowerCase() + "'").next();
        }
    ...
    public static boolean hasAccountMute(final String playerName) {
        try {
            return ACore.sql.getConnection().createStatement().executeQuery("SELECT * FROM `Mute` WHERE `username`='" + playerName.toLowerCase() + "'").next();
        }
        catch (SQLException ex) {
            ex.printStackTrace();
            return false;
    ...
    public static void loadBan(final String playerName) {
        ...
        try {
            final ResultSet result = ACore.sql.getConnection().createStatement().executeQuery("SELECT * FROM `Ban` WHERE `username`='" + playerName.toLowerCase() + "'");
            if (result.next()) {
    ...
    public static void loadMute(final String playerName) {
        ...
        try {
            final ResultSet result = ACore.sql.getConnection().createStatement().executeQuery("SELECT * FROM `Mute` WHERE `username`='" + playerName.toLowerCase() + "'");
            if (result.next()) {
                BanManager.addBan(result.getString("username"), result.getString("admin"), result.getString("reason"), result.getLong("time"), result.getString("comment"), Enum.valueOf(BanType.class, result.getString("type")), System.currentTimeMillis());
            }
        }
    ...
    public void loadBans() {
        try {
            ResultSet result = ACore.getInstance().getMySQL().getConnection().createStatement().executeQuery("SELECT * FROM `Ban`");
            while (result.next()) {
            ...
            result = ACore.getInstance().getMySQL().getConnection().createStatement().executeQuery("SELECT * FROM `Mute`");
            while (result.next()) {
    ...
    @Override
    public void run() {
        Bukkit.getScheduler().runTaskTimerAsynchronously((Plugin) ACore.getInstance(), () -> {
            try {
                ...
                ResultSet result2 = ACore.getInstance().getMySQL().getConnection().createStatement().executeQuery("SELECT * FROM `Mute`");
                while (result2.next()) {
